...
SOLR_PATH=/`;
    ...
    const typesenseModelSettings = searchEngine === 'typesense' ? `
        'model-settings' => [${(this.options.fromMain ? entities : [...entities, AcRule]).map(entity => `
            ${entity.name}::class => [
                'collection-schema' => [
                    'fields' => [
    ...
    if (searchEngine !== 'null') {
      this.fs.copyTpl(this.templatePath('server/config/scout.php.ejs'), this.destinationPath('server/config/scout.php'), {
        entities: this.options.fromMain ? entities : [...entities, AcRule],
    ...
    if (searchEngine === 'elastic') {
      ...
      for (const entity of entities) {
        const indexName = entity.tableName;
        const columns = entity.fields.reduce((res, field) => {
